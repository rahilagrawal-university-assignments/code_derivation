...
%
  ...
  \mathbf{con}\text{ } S: [10]^{*}\cdot{} s:\left[
    \begin{array}{l}
      x = \sum\limits_{i=0}^{c(n)}(S_i10^{(c(n)-1)}) \And n>0, s = \sum\limits_{i=0}^{c(n)}(S_i10^{i})
    \end{array}
  \right]}\\
%
  \refstep{\textbf{proc}}
...
\subsection{\color{blue}Implication 12\color{black}: $\myRefines{[\pre(19), s = \sum\limits_{i=0}^{c(n)}(S_i10^{i})]}{[x = \sum\limits_{i=0}^{c(n)}(S_i10^{(c(n)-1)}) \And n>0, s = \sum\limits_{i=0}^{c(n)}(S_i10^{i})]}$}
\begin{align*}
\end{align*}
